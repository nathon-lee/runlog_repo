...
class DeepSpeedPPOTrainer():
    ...
    def train_rlhf(self, inputs):
        ...
        # ===== [新增] 在最早阶段就检查 experience 是否含 NaN/Inf =====
        rank = torch.distributed.get_rank() if torch.distributed.is_initialized(
        ) else 0
        self._nancheck(log_probs, "old/log_probs", rank)
        self._nancheck(ref_log_probs, "old/ref_log_probs", rank)
        self._nancheck(reward_score, "old/reward_score", rank)
        self._nancheck(values, "old/values", rank)
        ...
        with torch.no_grad():
            ...
            self._nancheck(old_rewards, "old/old_rewards(after compute_rewards)", rank)
            ...
            self._nancheck(advantages, "old/advantages", rank)
            self._nancheck(returns, "old/returns", rank)
        ...
        self._nancheck(actor_log_prob, "new/actor_log_prob", rank)
        ...
        self._nancheck(value, "new/value", rank)

        critic_loss = self.critic_loss_fn(value[:, start:], old_values[:,
                                                                       start:],
                                          returns, action_mask[:, start:])

        self._nancheck(critic_loss, "loss/critic_loss", rank)
